...
all_star_b4::all_star_b4(QGraphicsItem *parent):QObject(),QGraphicsPixmapItem(parent)
{
       setPixmap(QPixmap(":/images/players/B(4).png"));
       Heading=qrand()%7;
       Vision=qrand()%7;
       Pace=qrand()%7;
...
void all_star_b4::move()
{
    ...
    QList<QGraphicsItem *> colliding_items=collidingItems();
         ...
         //qDebug()<<Pace<<" "<<Accelarlation<<endl;
if(Game->ContVar->StopB==4) return;
if(Game->ContVar->Half==1){
    if(Game->ContVar->Ball_Control==2){
             int Dir=rand()%8;

             if(Game->ContVar->TeamBside==2){
                 //qDebug()<<pos().x()<<" "<<pos().y()<<" "<<Dir<<endl;
                 if(Dir==0){
                     if(X>380)
