...
all_star_a10::all_star_a10(QGraphicsItem *parent):QObject(),QGraphicsPixmapItem(parent)
{

         setPixmap(QPixmap(":/images/players/A(10).png"));
         Heading=qrand()%7;
         Vision=qrand()%7;
         Pace=qrand()%7;
...
void all_star_a10::move()
{
   if(Game->ContVar->Freze_Player==1) return;
    ...
    //Game->ball->Own_Handle();setPos(pos().x()-(Pace+Accelarlation),pos().y());
                     Shot=rand()%4;
                     ...
                     if(Shot==0){
                         ...
                         if(Vision<4) Game->ball->Long_Shot(Game->NearB->Get_FarX()-5,Game->NearB->Get_FarY()-5);
                               ...
                               //Game->ball->Long_Shot_Air(Game->NearA->Get_FarX(),Game->NearA->Get_FarY());
 ...
 if(Game->ContVar->StopA==10) return;
 if(Game->ContVar->Half==1){
