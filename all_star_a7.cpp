...
all_star_a7::all_star_a7(QGraphicsItem *parent):QObject(),QGraphicsPixmapItem(parent)
{

         setPixmap(QPixmap(":/images/players/A(7).png"));
...
void all_star_a7::move()
{
    ...
    QList<QGraphicsItem *> colliding_items=collidingItems();
        ...
        /*     else if(Game->CC->Check_For_A(colliding_items[i])){
          ...
          if(Game->ContVar->StopA==7) return;
          if(Game->ContVar->Half==1){
              ...
              if(Game->ContVar->Ball_Control==1){
                       ...
                       if(Game->ContVar->TeamBside==2){
                          // qDebug()<<Pace<<"  "<<Accelarlation<<"  CR7"<<endl;
                           //qDebug()<<pos().x()<<" "<<pos().y()<<endl;
                           if(Dir==0){
                               if(X>15)
                               setPos(pos().x()-(Pace+Accelarlation)/2,pos().y());
